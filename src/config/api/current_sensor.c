/* ************************************************************************** */
/** Descriptive File Name

  @Company
    Company Name

  @File Name
    filename.c

  @Summary
    Brief description of the file.

  @Description
    Describe the purpose of this file.
 */
/* ************************************************************************** */
/* ************************************************************************** */
/* ************************************************************************** */
/* Section: Included Files                                                    */
/* ************************************************************************** */
/* ************************************************************************** */

#include "sys_parameter.h"

/* ************************************************************************** */
/* ************************************************************************** */
/* Section: File Scope or Global Data                                         */
/* ************************************************************************** */
/* ************************************************************************** */
#define CURR_SNSR_TYPE 1  // 0:MCP3421 1:CSNVC500

#define CURR_SNSR_TIMEOUT_TIME 10 /* unit:10ms */
#define IP_VALUE_OFFSET        0x80000000UL
#define ADC_COEFFICIENT        19966L
#define ADC_MOVING_AVG_TIMES   12L
#define ZERO_THRESHOLD         00L
unsigned char crc8Table[] = {
    ,
};

union {
    struct {
        unsigned char ipValue[4];
        unsigned      errorIndication : 1;
        unsigned      errorInfo       : 7;
        unsigned char vacantData[2];
        unsigned char crc8;
    };
    unsigned long long _8Bytes;
} DBC_CSNVC500;

static short         adcBuffer[ADC_MOVING_AVG_TIMES];
static unsigned char adcIndex      = 0;
static short         adcZeroOffset = 0;  // convert error value
short                adcGainOffset = 0;  // caculate error value
// TODO : static
unsigned int gIpValue              = 0;
unsigned int gCurrSnsrTimeoutCount = 0;
/* ************************************************************************** */
/* ************************************************************************** */
// Section: Interface Functions                                               */
/* ************************************************************************** */

/* ************************************************************************** */

static unsigned char Calculate_CRC8(unsigned char* ptr, unsigned char len) {
    unsigned char crc = 0x00;

    while (len--) {
        crc = crc8Table[crc ^ *ptr++];
    }
    return (crc);
}

/**
 * @brief      Transfer the ADC value to milli Ampere
 *
 * @param      adcValue
 * @return     int
 * @version    0.1
 * @author     Tu (Bamboo.Tu@amitatech.com)
 * @date       2022-09-06
 * @copyright  Copyright (c) 2022 Amita Technologies Inc.
 */
static int CURRSNSR_AdcValue2mAmpere(int adcValue) {
    int ret;
    int gain;
    /* Adjustment */
    adcValue += adcZeroOffset;
    /*                        EEPROM  */
    gain = ADC_COEFFICIENT + adcGainOffset;
    ret  = (adcValue * gain) / 1000;

    return ret;
}

/**
 * @brief      Get the moving average current value
 *
 * @param      arr
 * @param      arrSize
 * @return     int
 * @version    0.1
 * @author     Tu (Bamboo.Tu@amitatech.com)
 * @date       2022-09-06
 * @copyright  Copyright (c) 2022 Amita Technologies Inc.
 */
static inline int CURRSNSR_mAmpereGet(short* arr, unsigned int arrSize) {
    int ret = 0;
    ret     = (int)Filter_ArrayAverage(arr, arrSize);
    ret     = CURRSNSR_AdcValue2mAmpere(ret);
    ret     = ret;
    return ret;
}

/**
 * @brief      Set the current sensor's parameter
 *
 * @param      gainOffset
 * @param      zeroOffset
 * @version    0.1
 * @author     Tu (Bamboo.Tu@amitatech.com)
 * @date       2022-09-06
 * @copyright  Copyright (c) 2022 Amita Technologies Inc.
 */
void CURRSNSR_ParamSet(short gainOffset, short zeroOffset) {
    adcGainOffset = SATURATION(gainOffset, 1000, -1000);
    adcZeroOffset = SATURATION(zeroOffset, 1000, -1000);
}

/**
 * @brief      Initialize the current sensor's parameter after EEPROM read done
 *
 * @version    0.1
 * @author     Tu (Bamboo.Tu@amitatech.com)
 * @date       2022-09-06
 * @copyright  Copyright (c) 2022 Amita Technologies Inc.
 */
void CURRSNSR_Intialize(void) {
    CURRSNSR_ParamSet(eepSpe.AdcGainOffset, eepSpe.AdcZeroOffset);
}

/**
 * @brief      Decode the CSNVC500 current value
 *             The polling period is 10ms
 * @param      ipValue CSNVC500 current value(encode)
 * @return     int
 * @version    0.1
 * @author     Tu (Bamboo.Tu@amitatech.com)
 * @date       2022-09-26
 * @copyright  Copyright (c) 2022 Amita Technologies Inc.
 */
int CURRSNSR_CSNVC500(unsigned int ipValue) {
    int ret = 0;
    ret     = ipValue - IP_VALUE_OFFSET;
    return ret;
}

/**
 * @brief      Get the MCP3421 ADC value and then transfer the value to current value
 *             The polling period is 10ms
 * @version    0.1
 * @author     Tu (Bamboo.Tu@amitatech.com)
 * @date       2022-09-06
 * @copyright  Copyright (c) 2022 Amita Technologies Inc.
 */
void CURRSNSR_10ms_Tasks(void) {
    adcBuffer[adcIndex] = MCP3421_AdcValueGet();
    if (++adcIndex > ADC_MOVING_AVG_TIMES - 1) {
        adcIndex = 0;
    }
#if (CURR_SNSR_TYPE == 0)
    bmsData.BusCurrent = Filter_Deadzone(CURRSNSR_mAmpereGet(adcBuffer, ADC_MOVING_AVG_TIMES), ZERO_THRESHOLD);
#elif (CURR_SNSR_TYPE == 1)
    if (gCurrSnsrTimeoutCount) {
        gCurrSnsrTimeoutCount--;
    } else {
        gIpValue = IP_VALUE_OFFSET;
    }
    bmsData.BusCurrent = Filter_Deadzone(CURRSNSR_CSNVC500(gIpValue), ZERO_THRESHOLD);
#endif
}

void CURRSNSR_CheckQueueTask(can_msg_t canRxMsg) {
    gCurrSnsrTimeoutCount = CURR_SNSR_TIMEOUT_TIME;
    int tempData          = 0;

    if (Calculate_CRC8(canRxMsg.data, 7) == canRxMsg.data[7]) {
        tempData = (canRxMsg.data[3]) + (canRxMsg.data[2] << 8) + (canRxMsg.data[1] << 16) + (canRxMsg.data[0] << 24);
    }
    gIpValue = tempData;
}
